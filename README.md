...
- **실시간 미리보기**: 입력한 글씨가 실시간으로 화면에 표시

### 👨‍🎓 학생 모드
- **직관적인 글씨 연습**: 회색 가이드 글씨를 따라 쓰는 방식
- **실시간 라이브 평가**: 글씨를 쓰는 동안 즉시 피드백 제공
- **다양한 도구**: 연필/지우개 모드 지원
- **결과 저장**: 완성된 글씨를 이미지로 저장 가능

## 🎯 라이브 평가 시스템

학생이 글씨를 쓰는 동안 각 글자 칸 아래에 실시간 피드백이 표시됩니다:
...
### 등급 시스템
- **🐾 처음부터 차근차근** (0-24점): "이제 시작이에요! 하나씩 써보면 금방 나아질 거예요."
- **🌱 조금 더 연습해요!** (25-49점): "몇 글자는 잘 썼어요! 천천히 반복하면 더 좋아질 거예요."
- **🌸 잘했어요!** (50-79점): "거의 다 반듯하게 잘 썼어요! 조금 더 다듬으면 완벽해요."
- **🌟 정말 훌륭해요!** (80-100점): "글씨가 바르게 잘 써졌어요! 아주 멋진 실력이에요."
...
1. **모바일에서 사용 시**: 화면을 가로 모드로 회전하면 더 넓은 작업 공간 확보
2. **정확한 평가를 위해**: 회색 가이드선을 최대한 정확히 따라 써보세요
3. **실수했을 때**: 지우개 도구로 부분 수정하거나 🗑️ 모두 지우기로 처음부터 다시 시작
4. **과제 공유**: 생성된 링크는 언제든 재사용 가능합니다
